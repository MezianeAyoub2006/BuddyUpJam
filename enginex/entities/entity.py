...
class Entity(GameObject):
    ...
    def update(self, scene):
        colliding_rects = list(filter(lambda x : x.collide, scene.get_objects_by_tags("@Rect")))
        self.horizontal_physics(scene, colliding_rects)
        self.vertical_physics(scene, colliding_rects)
    ...
    def horizontal_physics(self, scene, colliding_rects):
        ...
        if self.collide:
            ...
            for rect_object in colliding_rects:
                if rect_object.rect.colliderect(rect):
                    if self.vel[0] > 0:
                        rect.right = rect_object.rect.left
                    if self.vel[0] < 0:
                        rect.left = rect_object.rect.right
                    self.pos[0] = rect.x - self.offset[0]

    def vertical_physics(self, scene, colliding_rects):
        ...
        if self.collide:
            for rect_object in colliding_rects:
                if rect_object.rect.colliderect(rect):
                    if self.vel[1] > 0:
                        rect.bottom = rect_object.rect.top
                    if self.vel[1] < 0:
                        rect.top = rect_object.rect.bottom
